...
import fetch$3 from 'cross-fetch';
...
function parseSuffix(suffix, containsLocator) {
  ...
  suffix = suffix.trim(); // If there is a label, the suffix must start with it
...
const readFile = async path => {
  if (isValidHttpUrl(path)) {
    return fetch$3(path).then(response => response.text()).then(data => data);
  } else {
    {
...
const getCitationFormat = citeproc => {
  ...
  const node = info.children.find(x => x['attrs'] && x['attrs']['citation-format']); // citation-format takes 5 possible values
  ...
  if (mode === 'note') {
    ...
    if (entries.length === 1) {
      ...
      for (const [i, item] of results.entries()) {
        ...
        const id = item.id;
        let citeMatch = item.ambig; // If author is the same as the previous, some styles like apa collapse the author
...
const rehypeCitationGenerator = Cite => {
  return (options = {}) => {
    return async (tree, file) => {
      ...
      if (isValidHttpUrl(bibliography)) {
        const response = await fetch$3(bibliography);
      ...
      visit(tree, 'text', (node, idx, parent) => {
        const match = node.value.match(citationRE); //@ts-ignore

        if (!match || !permittedTags.includes(parent.tagName)) return;
        let citeStartIdx = match.index;
        let citeEndIdx = match.index + match[0].length; // If we have an in-text citation and we should suppress the author, the
        ...
        const [entries, isComposite] = parseCitation(match); // If id is not in citation file (e.g. route alias or js package), abort process
        ...
        citationDict[citationId] = citedText; // Prepare citationPre and citationId for the next cite instance
      ...
      if (citeproc.registry.mylist.length >= 1 && (!options.suppressBibliography || ((_options$inlineBibCla = options.inlineBibClass) == null ? void 0 : _options$inlineBibCla.length) > 0)) {
        ...
        visit(tree, 'element', (node, idx, parent) => {
          var _options$inlineBibCla2, _node$properties2, _node$properties2$id;

          // Add inline bibliography
          if (((_options$inlineBibCla2 = options.inlineBibClass) == null ? void 0 : _options$inlineBibCla2.length) > 0 && (_node$properties2 = node.properties) != null && (_node$properties2$id = _node$properties2.id) != null && _node$properties2$id.toString().startsWith('citation-')) {
          ...
          if (node.tagName === 'sup') {
            ...
            if (nextNode.tagName === 'a') {
              ...
              if (href.includes('fn') && id.includes('fnref')) {
                ...
                nextNode.properties.href = `#user-content-fn-${index}`; // @ts-ignore
...
const types = {};
const dataTypes = {}; // extensions not registered as such
...
function type(input) {
  const dataType = dataTypeOf(input); // Empty array should be @csl/list+object too
  ...
  const match = matchType(dataTypes[dataType], input); // If no matching formats found, test if native format,
  // else invalid input.
...
function addTypeParser(format, {
  ...
  types[format] = object; // 3. determine which type lists the type should be added to
...
function removeTypeParser(type) {
  delete types[type]; // Removing orphaned type refs
...
function correctDate(date, bestGuessConversions) {
  ...
  if (typeof date !== 'object' || date === null) {
    return undefined; // "{'date-parts': [[2000, 1, 1], ...]}"
...
const URL$2 = Url.URL || publicApi.URL; // fix an issue where "format", "parse" aren't a named export for node <10
...
function fetch$2(url, opts) {
  ...
  Body$1.Promise = fetch$2.Promise; // wrap http.request into fetch
...
function fetch$1(resource, init) {
  ...
  } else if (resource instanceof URL$1) {
  ...
  request[1].headers = new lib.Headers(request[1].headers);
...
function extractContentType(input) {
  const request = new lib.Request(...input);
...
class SyncRequest extends lib.Request {
...
class SyncResponse extends lib.Response {
...
fetch$1.Headers = lib.Headers;
fetch$1.FetchError = lib.FetchError;
fetch$1.Request = SyncRequest;
fetch$1.Response = SyncResponse;
var syncFetch = fetch$1;
...
var fetchNode = function (context) {
  ...
  var fetchFn = lib.default ? lib.default : lib; // This modifies the global `node-fetch` object, which isn't great, since
  ...
  return function converter(input) {
    ...
    for (const {
      ...
      if (outputProp.length === 0) {
        continue; // Skip when requested by the requirements of the prop converter
      } else if (condition && !condition(input)) {
        continue; // Skip when none of the required props are in the input data
...
function prepareParseGraph(graph) {
  return graph // collapse continuous iterations of the same type
  .reduce((array, next) => {
...
function getJsonObject(src, dict) {
  ...
  if (isArray) {
    ...
    entries = Object.keys(src) // remove values that cannot be stringified, as is custom
...
const identifiers = ['PMID', 'PMCID', 'DOI', 'ISBN' // 'URL' is actually the URL of the record collection, if I understand it correctly,
...
const commands = unicode$2.commands; // Adapted from astrocite-bibtex (accessed 2018-02-22)
...
const valueGrammar = new Grammar({
  ...
  NameToken() {
    ...
    while (true) {
      ...
      if (upperCase === null && this.matchToken('text')) {
        ...
        value += text;
        upperCase = getStringCase(text); // If end of name part, return up
  ...
  BracketString() {
    ...
    let output = '';
    this.consumeToken('lbrace');
    const sentenceCase = this.state.sentenceCase; // If the bracket string starts with a command the sentence case behavior
    ...
    const topLevel = sentenceCase && !this.state.sentenceCase; // Protect the case of the bracket string if it is a direct child of the top
    // level, and the string is partly lowercase.

    const protectCase = topLevel && this.state.partlyLowercase; // Restore the sentence case of the outside of the brackets.
  ...
  Text() {
    ...
    this.state.afterPunctuation = /[?!.:]$/.test(text); // If the text fragment is not topLevel and has a case, check whether lowercase
    ...
    const restLowerCase = rest.toLowerCase(); // Word case should be preserved for proper nouns (e.g. those with capital
    ...
    return first + restLowerCase; // return first.toUpperCase() + restLowerCase
  },

  Command() {
    const commandToken = this.consumeToken('command');
    const command = commandToken.value; // formatting envs

    if (command in formattingEnvs) {
      const text = this.consumeRule('Env');
      const format = formattingEnvs[command];
      return applyFormatting(text, format); // formatting commands
    } else if (command in formattingCommands) {
      const text = this.consumeRule('BracketString');
      const format = formattingCommands[command];
      return applyFormatting(text, format); // commands
    } else if (command in commands) {
      return commands[command]; // diacritics
      ...
      return diacritic.normalize('NFC') + text.slice(1); // argument commands
      ...
      return func(...args); // escapes
    } else if (/^[&%$#_{}]$/.test(command)) {
      return commandToken.text.slice(1); // unknown commands
    } else {
